...
for idx, frame_path in enumerate(sorted(input_dir.glob("*.jpg"))):
    if idx >= max_frames:
        break
    ...
    for box in results.boxes:
        ...
        # Vẽ YOLO BBox: xanh dương
        cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 0, 0), 1)
        cv2.putText(frame, f"YOLO-{model.names[cls_id]}", (x1, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
    ...
    print(f"✅ {frame_path.name} | Detections: {len(detections)} | Tracks: {len(tracks)} | Time: {round(time.time() - start, 2)}s")
